...
| day_of_week マッピング | 1=月〜7=日 (ISO 8601) | ✓ |
| | 0=月〜6=日 | |
| 保存戦略 | delete + insert | ✓ |
...
**User's choice:** INT型・1=月〜7=日 (ISO 8601)・delete+insert・UIバリデーションのみ
**Notes:** 「INTで自然な表現でいいけど、週定義は月曜スタート日曜終わりなことに留意」と補足あり。ISO 8601 準拠（1=月〜7=日）に変更済み（`date-fns getISODay()` / `dayjs().isoWeekday()` と直接対応）。同一曜日の重複については「UIの話だよね？データの持ち方はとれるけど、UIとしては禁止したい」— UIで制約、DBにはUNIQUE制約なし
